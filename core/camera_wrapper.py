...
class LibCameraWrapper:
    def __init__(self, width, height, fps):
        ...
        if not self.cmd_base:
            # 开发环境 fallback
            raise ImportError("rpicam-vid 或 libcamera-vid 未找到，请安装 rpicam-apps。")

        logger.info(f"使用摄像头工具: {self.cmd_base}")

        # 检测 Pi 5 以应用特定修复
        self.is_pi5 = self._check_pi5()
        if self.is_pi5:
            logger.info("检测到 Raspberry Pi 5，已启用 'profile=baseline' 以禁用 B 帧。")

        self.udp_port = 1234
        self._start_pipeline()

    def _check_pi5(self) -> bool:
    ...
    def release(self):
        ...
        if self.process:
            try:
                ...
                logger.debug(f"杀死摄像头进程时出现非致命错误: {e}")
            self.process = None
        ...
        if hasattr(self, 'stderr_log') and self.stderr_log:
            try:
                ...
                logger.debug(f"关闭日志文件时出现非致命错误: {e}")
    ...
    def _start_pipeline(self):
        ...
        try:
            logger.info(f"正在启动摄像头进程: {' '.join(self.cmd)}")
            ...
            if self.process.poll() is not None:
                ...
                with open(self.log_file, "r") as f:
                    error_log = f.read()
                logger.error(f"摄像头进程启动失败，日志内容:\n{error_log}")
                raise RuntimeError("摄像头进程启动失败，请查看上方日志。")
            ...
            for i in range(max_retries):
                logger.info(f"正在连接 H.264 视频流 (第 {i+1}/{max_retries} 次尝试)...")
                self.cap = cv2.VideoCapture(udp_url, cv2.CAP_FFMPEG)

                if self.cap.isOpened():
                    logger.info("OpenCV 已成功连接摄像头视频流。")
                    return

                time.sleep(1.0)

            raise RuntimeError("多次重试后仍无法连接 H.264 视频流。")
    ...
    def _kill_existing_process(self) -> None:
        ...
        try:
            ...
            logger.debug(f"清理已有摄像头进程时出现非致命错误: {e}")

    def _handle_failure(self):
        """处理读取失败，必要时重启链路"""
        self.fail_count += 1
        if (self.process and self.process.poll() is not None) or self.fail_count >= self.max_fail:
            logger.warning("视频流中断，正在尝试重启摄像头流水线...")
            if self.process and self.process.poll() is not None:
                try:
                    self.stderr_log.seek(0)
                    logger.debug(f"最后的 stderr 输出: {self.stderr_log.read()[-500:]}")
                except Exception:
                    pass
